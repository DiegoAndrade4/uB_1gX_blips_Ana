// g++ metric_optimization_0nNn_EXT.cpp -o metric_optimization_0nNn_EXT $(root-config --cflags --libs)
#include <fstream>
#include <TFile.h>
#include <TTree.h>
#include <TVector3.h>
...
// Struct to hold vertex and momentum for each event
struct SinglePhotonEvent {
    TVector3 Shvertex;
    TVector3 Shmomentum;
};
// Function to load events from a ROOT file
std::vector<SinglePhotonEvent> LoadEvents(const char* filename = "events.root") {
    std::vector<SinglePhotonEvent> events;
    ...
    TTree* tree = (TTree*)inFile->Get("SPEventTree");
    if (!tree) {
        std::cerr << "Error: EventTree not found in file" << std::endl;
        inFile->Close();
        return events;
    }

    Double_t Sh_vx, Sh_vy, Sh_vz;
    Double_t Sh_px, Sh_py, Sh_pz;

    tree->SetBranchAddress("SPE_vertex_x", &Sh_vx);
    tree->SetBranchAddress("SPE_vertex_y", &Sh_vy);
    tree->SetBranchAddress("SPE_vertex_z", &Sh_vz);
    tree->SetBranchAddress("SPE_momentum_x", &Sh_px);
    tree->SetBranchAddress("SPE_momentum_y", &Sh_py);
    tree->SetBranchAddress("SPE_momentum_z", &Sh_pz);
    ...
    for (Long64_t i = 0; i < nEntries; i++) {
        tree->GetEntry(i);
        SinglePhotonEvent evt;
        evt.Shvertex.SetXYZ(Sh_vx, Sh_vy, Sh_vz);
        evt.Shmomentum.SetXYZ(Sh_px, Sh_py, Sh_pz);
    ...
    std::cout << "Loaded " << events.size() << " events from " << filename << std::endl;
    return events;
}
...
int Events_0n_all = 0 ; 
    ...
    int nevents = IsData ? 621  :  621 ;
//  int nevents = IsData ? fTree->GetEntries() * 0.5   :  fTree->GetEntries() ;
    std::cout << "fTree->GetEntries() " << fTree->GetEntries() << std::endl;
    std::cout << "Total events: " << nevents << std::endl;
// Load events from file
   std::vector<SinglePhotonEvent> events = LoadEvents("Run4b_CRT_BNBnu_Single_Photon_Events_ShowerVtx_ShowerDirection.root"); // Shower Vtx and Dir from BNBNu SPE
...
//                TVector3 ShowerMomentum(reco_showerMomentum[0], reco_showerMomentum[1], reco_showerMomentum[2]); // ShowerDir
               ...
               // Get a random event
               //int randomIndex = gRandom->Integer(events.size());
                //TVector3 ShVtx = events[randomIndex].Shvertex;
                //TVector3 ShowerMomentum = events[randomIndex].Shmomentum;
               //sequential
                TVector3 ShVtx = events[iEvent].Shvertex;
                TVector3 ShowerMomentum = events[iEvent].Shmomentum;

 int BacktrackedBlips = 0 ; 
int NBlips_sig_all_blips = 0, NBlips_sig_mc_blips = 0 , NBlips_sig_over_blips = 0 ; 
float   SumE_sig_all_blips = 0,   SumE_sig_mc_blips = 0 ,   SumE_sig_over_blips = 0 ; 

int NBlips_sig_all_regB_blips = 0 ; float  SumE_sig_all_regB_blips = 0 ; 
				     ...
				     //-- Blip section --
						...
						NBlips_sig_all_blips++ ; // <--NBlips_sig_all_regA_blips
				        	SumE_sig_all_blips += blip_energy->at(iBlip); // <--SumE_sig_all_regA_blips 
					 	...
					 	SumE_sig_over_blips += blip_energy->at(iBlip);
						      ...
						      NBlips_sig_all_regB_blips++ ; 
                                                      SumE_sig_all_regB_blips += blip_energy->at(iBlip); 
					...
					//regA and regB
                                    h2D_1gX_BlipMultiplicity_SumEblip_regA->Fill(NBlips_sig_all_blips, SumE_sig_all_blips);
                                    h2D_1gX_BlipMultiplicity_SumEblip_regB->Fill(NBlips_sig_all_regB_blips, SumE_sig_all_regB_blips);
...
// }//End Signal events
 ...
 std::cout <<"Total signal_event "<<signal_events<<std::endl;
    ...
    std::ofstream outfile(outDir + "/REN_pars_Nn0n_opt_EXT_BNBnuRef.txt", std::ios::app);

    if (!outfile.is_open()) std::cerr << "Error REN_pars_Nn0n_opt_EXT_BNBnuRef.txt for writing!\n";
